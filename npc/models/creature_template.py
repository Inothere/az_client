...
class CreatureTemplate(WorldBase):
    ...
    exp = sa.Column(ChoiceType(enums.ExpansionEnum, impl=sa.Integer()), 
        nullable=False, default=enums.ExpansionEnum.Classic)
    ...
    # npc品级，仅影响刷新时间
    rank = sa.Column(ChoiceType(enums.NpcRankEnum, impl=sa.Integer()), 
        default=enums.NpcRankEnum)
    # 近战伤害类型
    dmgschool  = sa.Column(
    ...
    DamageModifier = sa.Column(
        sa.Float(), default=1.0
    )
    # todo 
    HealthModifier = sa.Column(
        sa.Float(),
        default=1.0
    )
    # todo 
    ManaModifier = sa.Column(
        sa.Float(),
        default=1.0
    )
    # todo 
    ArmorModifier = sa.Column(
        sa.Float(),
        default=1.0
    )

    # 近战攻击间隔 ms
    BaseAttackTime = sa.Column(
        sa.Integer(), nullable=False, default=0
    )
    # 远程攻击间隔 ms
    RangeAttackTime = sa.Column(
        sa.Integer(), nullable=False, default=0
    )
    # 近战伤害计算相关
    BaseVariance = sa.Column(
        sa.Float(), default=1.0
    )
    # 远程伤害计算相关
    RangeVariance = sa.Column(
        sa.Float(), default=1.0
    )
    # npc职业
    ...
    trainer_type = sa.Column(
        sa.Integer(),
        default=0  # 0: 职业技能训练师，1: 骑术训练师，2: 专业技能训练师，3: 宠物训练师
    )
    trainer_spell = sa.Column(
        sa.Integer(),
        default=0  # trainer_type = 2时有效，必须指定是哪种专业技能ID
    )
    trainer_class = sa.Column(
        sa.Integer(),
        default=0 # trainer_type = 0, 3 时有效，必须指定职业ID
    )
    trainer_race = sa.Column(
        sa.Integer(),
        default=0  # trainer_type = 1时有效，坐骑训练师的种族ID必须和玩家种族ID相同
    )
